...
\begin{figure}[!ht]\includegraphics[trim={0cm 0cm 0 0},clip, keepaspectratio,width=0.9\textwidth]{YSOseq}\label{fig:YSOseq}\end{figure} 
...
\end{column}\begin{column}{0.5\textwidth}
\begin{figure}[!ht]\includegraphics[trim={0cm 0cm 0 0},clip, keepaspectratio,width=0.5\textwidth]{impactparam}\label{fig:impactparam}\end{figure}
\end{column}\end{columns}
\end{frame}
...
\begin{frame}{Dynamical friction: two body relaxation for particles with mass distribution}
Equipartition of random energy of planetesimal: $v\propto m\expy{1/2}$; for $M\gg m$, $e_M>e$, dynamical friction rate
\[\TDy{t}{e_M^2}\approx-\frac{m}{M}\frac{e^2_M}{t_{rel}}\]
\begin{columns}\begin{column}{0.5\textwidth}

\end{column}\begin{column}{0.5\textwidth}
V-like structure in a-e plane corresponds to constant Jacobi energy curve
\end{column}\end{columns}
\end{frame}

\begin{frame}{Growth vs fragmentation}
lectures pg 41
\end{frame}
...
\begin{frame}{Accretion vs scattering}
Velocity difference planet/planetesimal: orbite distanti $\Delta a$ abbastanza vicine da arrivare a incontro. $\Delta a\propto r_h$: $\Delta v=v_r\propto|\TDy{a}{v_K}|r_h\approx\sqrt{\frac{GM_*\expy{1/3}M_p\expy{2/3}}{a}}$. If $\Delta v\gg v_{esc}$ it's impossible for planet to scatter planetesimal into different orbit, $\Delta v\ll v_{esc}$ scatter dominates over collisions.
Fissato a: i planetesimi si incontrano nella fascia $\exv{e}a$, $\exv{i}a$.
Densit\'a numerica di planetesimi: $\frac{\Sigma_pn}{m_{pl}v_r}$.
Urto con parametro d'impatto b:
\[\delta v_r=\underbrace{\frac{Gm_{pl}}{b^2}}_{\exv{a}}\underbrace{\frac{2b}{v_r}}_{\tau}=\frac{2Gm_{pl}}{bv_r}\]
\end{frame}

\begin{frame}{Evoluzione $v_r$: feedback scattering e collisioni}
\begin{itemize}
...
$v_r\approx ena$: a e eccentricit\'a media e i corpi si incontrano se compresi in fascia di larghezza ae.
...
\begin{equation*}
[\TDy{t}{v_r^2}]_{enc}=\int_{r_{pl}}^d2\pi bv_r\frac{\sigma n}{m_{pl}v_r}(\frac{2Gm_{pl}}{bv_r})^2\,db=2\pi\frac{\sigma n}{m_{pl}}(\frac{2Gm_{pl}}{v_r})^2\ln{\frac{d}{r_{pl}}}
\end{equation*}
\end{itemize}
